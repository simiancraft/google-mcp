...
/** The `fields` selection backing the User projection on authors. */
const USER_FIELDS = 'displayName,emailAddress,me,photoLink';
...
export function projectReply(data: drive_v3.Schema$Reply): Reply {
  return {
    ...
    action: narrow(data.action, ['resolve', 'reopen']),
  };
}
