...
export const schema = {
  input: z
    ...
    .refine((args) => args.textContent === undefined || args.base64Content === undefined, {
      message: 'textContent and base64Content cannot both be set.',
      path: ['base64Content'],
    })
    .refine(
      (args) =>
        args.contentMimeType !== undefined ||
        (args.textContent === undefined && args.base64Content === undefined),
      {
        message: 'contentMimeType is required when providing content.',
        path: ['contentMimeType'],
      },
    ),
  output: File,
};
