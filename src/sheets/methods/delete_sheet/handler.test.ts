...
describe('delete_sheet', () => {
  it('deletes the sheet and confirms the ids', async () => {
    const captured: Captured = {};
    const result = await handler(fakeSheets(captured), { spreadsheetId: 'SS', sheetId: 42 });
    expect(captured.params).toEqual({
      spreadsheetId: 'SS',
      requestBody: { requests: [{ deleteSheet: { sheetId: 42 } }] },
    });
    expect(result).toEqual({ spreadsheetId: 'SS', sheetId: 42 });
