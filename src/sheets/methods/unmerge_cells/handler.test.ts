...
describe('unmerge_cells', () => {
  it('unmerges every merge in the range', async () => {
    const captured: Captured = {};
    const result = await handler(fakeSheets(captured), {
      spreadsheetId: 'SS',
      range: { sheetId: 3, startRowIndex: 0, endRowIndex: 2 },
    });
    expect(captured.params).toEqual({
      spreadsheetId: 'SS',
      requestBody: {
        requests: [{ unmergeCells: { range: { sheetId: 3, startRowIndex: 0, endRowIndex: 2 } } }],
      },
    });
