...
describe('delete_paragraph_bullets', () => {
  it('removes bullets from the range', async () => {
    const captured: Captured = {};
    const result = await handler(
      fakeDocs(captured, { documentId: 'D1', writeControl: { requiredRevisionId: 'rev-7' } }),
      { documentId: 'D1', range: { startIndex: 1, endIndex: 30 } },
    );
    expect(captured.params).toEqual({
      documentId: 'D1',
      requestBody: {
        requests: [{ deleteParagraphBullets: { range: { startIndex: 1, endIndex: 30 } } }],
      },
    });
