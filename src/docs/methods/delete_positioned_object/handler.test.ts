...
describe('delete_positioned_object', () => {
  it('deletes by object id', async () => {
    ...
    expect(captured.params).toEqual({
      documentId: 'D1',
      requestBody: { requests: [{ deletePositionedObject: { objectId: 'kix.po1' } }] },
    });
    expect(result).toEqual({ documentId: 'D1' });
    expect(() => schema.output.parse(result)).not.toThrow();
  });

  it('rejects an empty objectId at the schema', () => {
    expect(schema.input.safeParse({ documentId: 'D2', objectId: '' }).success).toBe(false);
  });
});
